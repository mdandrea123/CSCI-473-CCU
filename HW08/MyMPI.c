   ...
   MPI_Comm     comm)     /* IN - Communicator */
{
   ...
   if (id == (p-1)) {
      ...
      if (infileptr == NULL) *m = 0;
         ...
         fread(m, sizeof(int), 1, infileptr);   // Read number of rows
         fread(n, sizeof(int), 1, infileptr);   // Read number of columns
      }      
   }
   MPI_Bcast(m, 1, MPI_INT, p-1, comm);
   if (!(*m)) MPI_Abort(MPI_COMM_WORLD, OPEN_FILE_ERROR);
   ...
   /* Process p-1 reads blocks of rows from the file and
      sends each block to the correct destination process. */

   if (id == (p-1)) {
      for (i = 0; i < p-1; i++) {
         x = fread((*subs)[0], datum_size, BLOCK_SIZE(i, p, *m) * *n, infileptr);
         if (x != BLOCK_SIZE(i, p, *m) * *n) {
            fprintf(stderr, "Error reading from file on process %d\n", id);
            MPI_Abort(MPI_COMM_WORLD, OPEN_FILE_ERROR);
         }
         MPI_Send((*subs)[0], BLOCK_SIZE(i, p, *m) * *n, dtype, i, DATA_MSG, comm);
      }
      x = fread((*subs)[0], datum_size, local_rows * *n, infileptr);
      if (x != local_rows * *n) {
         fprintf(stderr, "Error reading from file on process %d\n", id);
         MPI_Abort(MPI_COMM_WORLD, OPEN_FILE_ERROR);
      }
      ...
      MPI_Recv((*subs)[0], local_rows * *n, dtype, p-1, DATA_MSG, comm, &status);
   }
}
