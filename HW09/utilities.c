...
int get_local_rows_with_halo(int rank, int size, int m) {
    ...
    if (size == 1) {
        return local_rows;  // No halos needed
    } else if (rank == 0) {
        return local_rows + 1;  // Only bottom halo
    } else if (rank == size - 1) {
        return local_rows + 1;  // Only top halo
    } else {
        return local_rows + 2;  // Both halos
    }
}
    ...
    MPI_Comm comm)      /* IN - Communicator */
{
    ...
    int real_rows = BLOCK_SIZE(rank, size, *m);
    int total_rows = get_local_rows_with_halo(rank, size, *m);
    
    // Allocate space including halos
    *subs = (void **)malloc(total_rows * sizeof(void *));
    (*subs)[0] = malloc(total_rows * (*n) * sizeof(double));
    
    // Set up row pointers
    for (int i = 1; i < total_rows; i++) {
        (*subs)[i] = (*subs)[0] + i * (*n) * sizeof(double);
    }
    
    // Initialize halo rows to -1
    double **matrix = (double **)*subs;
    if (rank > 0) {  // Initialize top halo
        for (int j = 0; j < *n; j++) {
            matrix[0][j] = -1.0;
        }
    }
    if (rank < size - 1) {  // Initialize bottom halo
        for (int j = 0; j < *n; j++) {
            matrix[total_rows - 1][j] = -1.0;
        }
    }
    ...
    if (rank == (size-1)) {
        ...
        // Read and distribute to other processes
        ...
        // Read own portion
        int start_row = 1;  // Skip halo row
        fread(&matrix[start_row][0], sizeof(double), real_rows * (*n), f);
        fclose(f);
    } else {
        // Receive data from last process
        int start_row = (rank == 0) ? 0 : 1;  // Skip halo row if not process 0
        MPI_Recv(&matrix[start_row][0], real_rows * (*n), 
...
void exchange_row_striped_matrix_halo(
    void **a,           /* IN/OUT - 2D array with halo rows */
    MPI_Datatype dtype, /* IN - Matrix element type */
    int rows,           /* IN - Total original matrix rows (without halos) */
    ...
    MPI_Comm comm)      /* IN - Communicator */
{
    ...
    int real_rows = BLOCK_SIZE(rank, size, rows);
    MPI_Status status;
    
    // First process only sends down and receives from below
    if (rank == 0) {
        // Send bottom real row to next process's top halo
        MPI_Send(&matrix[real_rows - 1][0], cols, dtype, rank + 1, 0, comm);
        // Receive into bottom halo from next process
        MPI_Recv(&matrix[real_rows][0], cols, dtype, rank + 1, 1, comm, &status);
    }
    // Last process only sends up and receives from above
    else if (rank == size - 1) {
        // Send first real row to previous process's bottom halo
        MPI_Send(&matrix[1][0], cols, dtype, rank - 1, 1, comm);
        // Receive into top halo from previous process
        MPI_Recv(&matrix[0][0], cols, dtype, rank - 1, 0, comm, &status);
    }
    // Middle processes need to exchange with both neighbors
    else {
        // First exchange with the process above
        MPI_Sendrecv(&matrix[1][0], cols, dtype, rank - 1, 1,
                     &matrix[0][0], cols, dtype, rank - 1, 0,
                     comm, &status);
        
        // Then exchange with the process below
        MPI_Sendrecv(&matrix[real_rows][0], cols, dtype, rank + 1, 0,
                     &matrix[real_rows + 1][0], cols, dtype, rank + 1, 1,
                     comm, &status);
    }
}
